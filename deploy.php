 ...
 * @var string
 */
if (!defined('REMOTE_REPOSITORY')) define('REMOTE_REPOSITORY', 'https://github.com/iswebdevelopers/Fusioncorporate-portal.git');
 ...
 * The options that the composer is going to use.
 *
 * @var string Composer woring direcotry
 * @link https://getcomposer.org/doc/03-cli.md#global-options
 */
if (!define('COMPOSER_WORKING_DIR')) define('COMPOSER_WORKING_DIR', false);
...
$requiredBinaries = array('git', 'robocopy');
...
// if (defined('BACKUP_DIR') && BACKUP_DIR !== false) {
// 	$commands[] = sprintf(
// 		"tar --exclude='%s*' -cvf %s/%s-%s-%s.tar.gz -C %s*"
// 		, BACKUP_DIR
// 		, BACKUP_DIR
// 		, basename(TARGET_DIR)
// 		, md5(TARGET_DIR)
...
if (defined('USE_COMPOSER') && USE_COMPOSER === true) {
	...
	$commands[] = sprintf(
		'composer --no-ansi --no-interaction --no-progress --working-dir=%s update %s'
...
// Artisan command
$commands[] = sprintf(
	'cd %s && php artisan cache:clear'
	, COMPOSER_WORKING_DIR
);
$commands[] = sprintf(
	'cd %s && php artisan view:clear'
	, COMPOSER_WORKING_DIR
);
...
foreach ($commands as $command) {
	...
	$tmp = array();
	exec($command.' 2>log.txt', $tmp, $return_code); // Execute the command
	// Output the result
	printf('
	...
	if (($return_code !== 0) && (strpos($command, "robocopy") === false )) {
		header($_SERVER['SERVER_PROTOCOL'] . ' 500 Internal Server Error', true, 500);
		printf($return_code .'
			<div class="error">
			Error encountered!
		...
		if (EMAIL_ON_ERROR) {
			...
			try {
				$mail = mail(EMAIL_ON_ERROR, $error, strip_tags(trim($output)), implode("\r\n", $headers));
				echo "email sent";
			} catch (Exception $e) {
				dd($e);
			}
		...
		if($return_code > 7) {
			...
			printf($return_code .'
				<div class="error">
				Error on robocopy command encountered!
				Stopping the script to prevent possible data loss.
				CHECK THE DATA IN YOUR TARGET DIR!
				</div>
				'
